    ...
    public class ImageUtilsTest : ITest
    {
        public bool Test()
        {
            return
                ...
                TestImageUtils(
                    "http://shutterstoppers.com/wp-content/uploads/2014/08/landscape-photo5-150x150.jpg");
        }

        private static bool TestImageUtils(string url)
        {
            var image = new WebClient().DownloadData(url);
            var imageString = image.ByteArrayToString();
            var newImage = imageString.ToByteArray();
            var equal = true;
